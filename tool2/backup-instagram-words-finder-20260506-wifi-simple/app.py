...
def iter_profile_content(profile: instaloader.Profile, per_source_limit: int = 0):
    seen = set()
...
def search():
    ...
    try:
        ...
        scanned = 0
        matches: list[dict] = []
        per_source_limit = max(1, max_posts // 2) if max_posts else 0
        for source, post in iter_profile_content(profile, per_source_limit):
            scanned += 1
            caption = post.caption or ""
...
def stream_search_posts(
    ...
    try:
        ...
        per_source_limit = max(1, scan_limit // 2) if scan_limit else 0
        for source, post in iter_profile_content(profile, per_source_limit):
            scanned += 1
...
def search_stream():
    ...
    scan_limit = compact_number(request.args.get("scan_limit")) or DEFAULT_SCAN_LIMIT
    match_limit = compact_number(request.args.get("match_limit")) or DEFAULT_MATCH_LIMIT
    time_limit = compact_number(request.args.get("time_limit")) or DEFAULT_SEARCH_SECONDS
